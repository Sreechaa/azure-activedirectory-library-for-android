...
class BrokerProxy implements IBrokerProxy {
	 ...
	 * ADAL directs call to AccountManager if component is valid, present, and
	 * There should be an account with broker account type. It does not direct
	 * call if the caller is from Authenticator itself.
	 */
	@Override
	public boolean canSwitchToBroker() {
		return !mContext.getPackageName().equalsIgnoreCase(
				...
				&& verifyBroker()
				&& verifyAuthenticator() /**&& anyAccount()**/;
	}
	...
	private boolean verifyManifestPermissions() {
		Logger.v(TAG, "Verify permissions in the manifest file");
	 ...
	 */
	@Override
	public AuthenticationResult getAuthTokenInBackground(
		...
		Account targetAccount = getAccount(accountList,
		...
		if (targetAccount != null) {
			...
			try {
				...
				result = mAcctManager.getAuthToken(targetAccount,
						...
						brokerOptions, false, null /*
													 * set to null to avoid
													 * callback
													 */, mHandler);
	...
	private AuthenticationResult getResultFromBrokerResponse(Bundle bundleResult) {
		...
		if (!StringExtensions.IsNullOrBlank(msg)) {
			...
			String accountName = bundleResult
			...
			UserInfo userinfo = UserInfo
					.getUserInfoFromBrokerResult(bundleResult);
	...
	public void saveAccount(String accountName) {
		...
		String accountList = prefs.getString(
				KEY_APP_ACCOUNTS_FOR_TOKEN_REMOVAL, "");
		if (!accountList.contains(KEY_ACCOUNT_LIST_DELIM + accountName)) {
			accountList += KEY_ACCOUNT_LIST_DELIM + accountName;
			Editor prefsEditor = prefs.edit();
			prefsEditor.putString(KEY_APP_ACCOUNTS_FOR_TOKEN_REMOVAL,
					accountList);
	...
	public void removeAccounts() {
		new Thread(new Runnable() {
			...
			public void run() {
				...
				SharedPreferences prefs = mContext.getSharedPreferences(
						KEY_SHARED_PREF_ACCOUNT_LIST, Activity.MODE_PRIVATE);
				String delAccount = prefs.getString(
						KEY_APP_ACCOUNTS_FOR_TOKEN_REMOVAL, "");
				StringTokenizer st = new StringTokenizer(delAccount,
						KEY_ACCOUNT_LIST_DELIM);
	...
	public Intent getIntentForBrokerActivity(final AuthenticationRequest request) {
		...
		try {
			...
			result = mAcctManager.addAccount(
					AuthenticationConstants.Broker.BROKER_ACCOUNT_TYPE,
					AuthenticationConstants.Broker.AUTHTOKEN_TYPE, null,
					addAccountOptions, null, null, mHandler);
	...
	private Bundle getBrokerOptions(final AuthenticationRequest request) {
		...
		brokerOptions.putInt(AuthenticationConstants.Browser.REQUEST_ID,
	...
	private boolean verifyBroker() {
		try {
			Logger.v(TAG, "Verify broker signature");
			...
			if (info != null && info.signatures != null) {
				...
				for (Signature signature : info.signatures) {
					...
					String tag = Base64.encodeToString(md.digest(),
							Base64.DEFAULT);
	...
	private boolean verifyAuthenticator() {
		...
		Logger.v(TAG, "Verify authenticator instances in the account manager");
		AuthenticatorDescription[] authenticators = mAcctManager
				.getAuthenticatorTypes();
		for (AuthenticatorDescription authenticator : authenticators) {
			if (authenticator.packageName
					.equals(AuthenticationSettings.INSTANCE
	...
	private boolean anyAccount() {
		Account[] accountList = mAcctManager
				.getAccountsByType(AuthenticationConstants.Broker.BROKER_ACCOUNT_TYPE);
		if (accountList != null && accountList.length > 0) {
			Logger.v(TAG, "Number of accounts:" + accountList.length);
			return true;
		}
	...
	public ArrayList<String> getAccounts() {
        ...
        if (accountList != null && accountList.length > 0) {
            ...
            for (Account accountEntry : accountList) {
                accountNames.add(accountEntry.name);
            }
        }
	...
	public boolean hasAccount(String accountName) {
		...
		if (accountList != null && accountList.length > 0) {
			Logger.v(TAG, "Number of accounts:" + accountList.length);
			for (Account accountEntry : accountList) {
				if (accountEntry.name.equalsIgnoreCase(accountName)) {
					return true;
				}
			}
		}
